...
const models = ['z-ai/glm-5.3', 'deepseek/deepseek-v4-flash-0731', 'z-ai/glm-5.2', 'moonshotai/kimi-k3', 'qwen/qwen3.6-27b', 'google/gemma-4-31b-it'];
...
async function request(model:string,test:string,messages:any[],extra:any={}) {
 ...
 const row:any={sequence:++sequence,model,test,startedAt:new Date().toISOString(),request:{messages,...extra},httpStatus:null,transportPass:false,frames:0,firstEventMs:null,firstContentMs:null,firstToolMs:null,text:'',reasoningCharacters:0,done:false,finishReason:null,usage:null,calls:[]};
...
async function runRound(name:string,handler:(model:string)=>Promise<void>,round:number) {
 ...
 for(let i=0;i<order.length;i+=2)await Promise.all(order.slice(i,i+2).map(handler));
}
await writeFile(new URL('results.jsonl',dir),'');
await writeFile(new URL('method.json',dir),JSON.stringify({startedAt,endpoint,models,requestsPerModel:6,maxConcurrency:2,requestTimeoutMs:60000,maxTokens:8192,defaultSampling:true,synthesisReasoningEffort:'low',circuitBreaker:'Stop after two transport failures for a model; honor Retry-After. No immediate retries.',meetingPrompt,mathPrompt,mathGold,toolPrompt,notes:['Synthetic public fixtures only; no user conversations.','First content time measures visible text, not hidden reasoning.','Transport success and exact fixture correctness are reported separately.','These short samples cannot establish long-term reliability or broad quality ranking.','Provider telemetry and price claims are snapshots, not measured request success.','Tools simulate a local lookup; this does not validate live Eliza authorization or dispatch.','No per-response attestation verification or large-context tests.']},null,2)+'\n');
await runRound('short-reply-1',async model=>{const r=await request(model,'short-reply-1',[{role:'user',content:'Reply exactly READY_731 and nothing else.'}]);r.assessment={pass:r.transportPass&&r.text.trim()==='READY_731'};await save(r);},0);
...
await runRound('short-reply-2',async model=>{const r=await request(model,'short-reply-2',[{role:'user',content:'Reply exactly READY_946 and nothing else.'}]);r.assessment={pass:r.transportPass&&r.text.trim()==='READY_946'};await save(r);},4);
