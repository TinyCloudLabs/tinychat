...
test.skipIf(!existsSync(new URL("server.ts", elizaSource)))("actual TaskHandler access denial remains a valid failed final without adding a browser delegation code (requires sibling Eliza checkout)", async () => {
  ...
  const lease = sessions.reserve(app, entityId)!;
  sessions.commit(app, entityId, lease, { agentId: TINYCHAT_AGENT_ID, serializedDelegation: "controlled-fixture" });
  let providers = 0;
  let actions = 0;
  ...
  } }, { privateAccessAvailable: () => true, runtimeFor: async () => ({ actions: [{ name: "TINYCLOUD_READ_MEETING", handler: async () => { actions++; throw new ToolError("fixture denied", 403, "access_denied"); } }] }) as any }, sessions);
  const codes: string[] = [];
