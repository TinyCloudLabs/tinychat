...
describe("pendingHandoff per-message keying (ST4)", () => {
  ...
  test("terminal claim is immutable and restored message bindings preserve parent order", async () => {
    const { createTurnOutcomeStore } = await import('./pendingHandoff');
    const store = createTurnOutcomeStore();
    store.begin('thread', 'user', 1);
    expect(store.claim('thread', { turnId: 'user', sentAt: 1, private: true, status: 'cancelled' })).toBe(true);
    expect(store.claim('thread', { turnId: 'user', sentAt: 1, private: false, status: 'completed' })).toBe(false);
    store.bind('thread', 'assistant', store.get('thread', 'user')!);
    expect(store.forMessage('thread', 'assistant')?.status).toBe('cancelled');
    expect(store.forMessage('other', 'assistant')).toBeUndefined();
  });
});
